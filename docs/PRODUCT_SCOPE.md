...
- **Structured output**: JSON and SARIF with run metadata (`duration_ms`, `files_scanned`, `bytes_scanned`, `patterns_active`, diagnostics) for CI and Code Scanning. Integrators deciding pass/fail must read `run.blocking_matches` (findings at or above the effective `fail_on` threshold; this is what sets vault-guard's own exit code). `summary.secrets` is only a total finding count across the whole run, threshold or no threshold, and is not a pass/fail signal. `run.unscannable_files` (present when non-zero) counts files that were reached but could not be read; while it is set, a clean `blocking_matches` covers only the files that were actually examined. `run.baseline_suppressed` and `run.inline_suppressed` report how many findings were hidden by the baseline and by inline `vault-guard: ignore-line` / `ignore-next-line` directives; `inline_suppressed` is emitted every run (even at zero), and each file that suppressed a finding inline also carries a `suppression.inline` diagnostic naming the line numbers, because a suppression is the user's decision and must stay visible.
